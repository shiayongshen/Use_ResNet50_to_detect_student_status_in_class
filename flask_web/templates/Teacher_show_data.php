...
include('connectMySQL.php');
	...
	$query = "SELECT id, name, time, face, rppg FROM students S JOIN face_data F JOIN data D 
                ON S.tip = F.tip and S.tip = D.tip WHERE login = '1'"; 
		...
		<tbody>
			...
			<?php
				if(mysqli_num_rows($query_run) > 0)
				{
					foreach($query_run as $row){
                        //if(($row['face'] == 'confuse' and $row['rppg'] <= 1) or ($row['face'] == 'sleepy' and $row['rppg'] <= 1)) {
			...
			?>
							<tr>
                                ...
                                <td><?php 
                                        if ($row['face'] == 'confuse' and $row['rppg'] <= 1){
                                                    echo "<font color=red>不專注</font>";}
                                        else if ($row['face'] == 'sleepy' and $row['rppg'] <= 1){
                                                    echo "<font color=red>不專注</font>";}
                                        else if ($row['face'] == 'normal' and $row['rppg'] > 1){
                                                    echo "<font color=green>專注</font>";}
                                        else if ($row['face'] == 'happy' and $row['rppg'] > 1){
                                                    echo "<font color=green>專注</font>";}
                                        else {echo "<font color=orange>橘燈</font>";}
                                ?></td>
                                
                                <td><?php echo $row['face'];?></td>
                                <td><?php echo $row['rppg'];?></td>
							</tr>
			<?php
				  }
				}
